...
struct D3DNVGcontext {
	...
	ID3D11BlendState* pBSBlendAdditive;
...
static int D3Dnvg__renderCreate(void* uptr)
{
	...
	hr = D3D_API_2(D3D->pDevice, CreateBlendState, &blendDesc, &D3D->pBSBlendAdditive);
...
static void D3Dnvg__fill(struct D3DNVGcontext* D3D, struct D3DNVGcall* call, ID3D11BlendState* blendState)
{
	...
	D3D_API_3(D3D->pDeviceContext, OMSetBlendState, blendState, NULL, 0xFFFFFFFF);
...
static void D3Dnvg__renderFlush(void* uptr, int blendMode)
{
	struct D3DNVGcontext* D3D = (struct D3DNVGcontext*)uptr;
	int i;
	...
	if (D3D->ncalls > 0) 
    {
        ...
        D3D_API_3(D3D->pDeviceContext, VSSetShader, D3D->shader.vert, NULL, 0);
         ...
         // Draw shapes
		ID3D11BlendState* const blendState = blendMode == NVGblendModes::NVG_BLEND_MODE_ADDITIVE ? D3D->pBSBlendAdditive : D3D->pBSBlend;
        D3D_API_2(D3D->pDeviceContext, OMSetDepthStencilState, D3D->pDepthStencilDefault, 0);
		D3D_API_3(D3D->pDeviceContext, OMSetBlendState, blendState, NULL, 0xFFFFFFFF);
		...
		for (i = 0; i < D3D->ncalls; i++) {
			...
			if (call->type == D3DNVG_FILL)
				D3Dnvg__fill(D3D, call, blendState);
...
static void D3Dnvg__renderDelete(void* uptr)
{
	...
	D3D_API_RELEASE(D3D->pBSBlendAdditive);
