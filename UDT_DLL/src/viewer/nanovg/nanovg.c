...
{
	ctx->params.renderFlush(ctx->params.userPtr, nvg__getState(ctx)->blendMode);
...
void nvgReset(NVGcontext* ctx)
{
	...
	state->fontId = 0;
	state->blendMode = NVG_BLEND_MODE_NORMAL;
}
...
void nvgBlendMode(NVGcontext* ctx, int blendMode)
{
	nvg__getState(ctx)->blendMode = blendMode;
}
